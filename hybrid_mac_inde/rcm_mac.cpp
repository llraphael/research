...
int main() 
{
  ...
  for(int i=0;i<weightsize;i=i+2) {
      weightset[i] = wval[i / 2];
      weightset[i+1] = -wval[i / 2];
  }
  ...
  string errorFileName = "rcm_mac_inde_w4142_" + std::to_string(static_cast<long long>(symbolNumber)) + "_" + std::to_string(static_cast<long long>(sysDegree)) + "_p" + pString + "_" + gapString + "db.txt";
  ...
  while(currentblock < blocknum) {
    ...
    while(currenttime != iterationtime+1) {
      ...
      for(int i=0;i<sysNumber;++i)
        sysNodeInfo1[i].computeMessage(symNodeInfo1, sysChannelOutput[i], sideInfo[i]);
      ...
      for(int i=0;i<sysNumber;++i)
        sysNodeInfo2[i].computeMessage(symNodeInfo2, sysChannelOutput[i], sideInfo[i]);
