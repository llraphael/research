...
double deconvolve2(std::vector<double> &sumpdf, std::vector<double> &vpdf, double weight, std::vector<double> &respdf); // If x has probability mass at 0, 1, 2, then the deconvolution method would change to this one.
...
std::vector<std::vector<double> > syntheticDecoderRP(Coded_info &symNodeInfo1, Coded_info &symNodeInfo2, int symIndex, std::vector<Sys_info>& sysNodeInfo1, std::vector<Sys_info>& sysNodeInfo2, std::vector<double>& channelPdf, double norFactor);
...
// LDGM generator matrix generator
std::vector<std::multimap<int, int> > paritycm(int sysNo, int codedNo, int sysDegree, int numWithSysDegree, int seed);
#endif
