...
int main() 
{
  ...
  int currentBlock = 0;
    ...
    int sametime = 0;
		...
		// Message passing for RCM part.
		int RCMIteration = 3;
		cout << "RCM iterations is running." << endl;
		for(int ite=0; ite<RCMIteration; ite++) {
			for(int i=0; i<symbolNumber; i++)
				pdfMessageFromRP[i] = syntheticDecoderRP(symNodeInfo1[i], symNodeInfo2[i], i, pdfMessageFromSys, channelPdf[i], norFactor);
			
			for(int i=0; i<sysNumber; i++) {
				pdfMessageFromSys[i] = syntheticDecoderSys(sysNodeInfo1[i], sysNodeInfo2[i], pdfMessageFromRP, sysChannelOutput[i], p, i, trackRPPro, systrans1[i], systrans2[i], ite);
			}

			int count = 0;
			if(sysNodeInfo1[0].decision == 1) {
				count += countError(sysNodeInfo1, systrans1);
				count += countError(sysNodeInfo2, systrans2);
      } else if(sysNodeInfo1[0].decision == 0) {
				count += countError(sysNodeInfo1, systrans2);
				count  += countError(sysNodeInfo2, systrans1);
      }

			cout << "RCM error number: " << count << " (" << ite + 1<< ")" << endl;
		}
  
		vector<double> sysInitialLLR(sysNumber);
		for(int i=0; i<sysNumber; i++) 
			sysInitialLLR[i] = sysNodeInfo1[i].llrEstimation;

	 	// Message passing for LDGM codes.
      ...
      ++currentTime;
			...
			for(int i=0; i<sysNumber; i++)
				sysNodeInfo1[i].computeMessage(codedNodeInfo1, sysInitialLLR[i],0, 2);	
			for(int i=0; i<codedNumber; i++) 
				codedNodeInfo1[i].computeMessage(sysNodeInfo1, codedChannelOutput[i], 2);
			...
			for(int i=0; i<sysNumber; i++) {
				...
				if(isinf(nominator1) && isinf(denominator1)) {
					sysInitialLLR[i] = log(exp(-pow(y, 2) / (2 * pow(sigma, 2))) / exp(-pow(y - 2 * sqrt(Esender), 2) / (2 * pow(sigma, 2))));
				} else {
					sysInitialLLR[i] = log((nominator1 + nominator2) / (denominator1 + denominator2));
				}

				if(currentTime == 1) {
					for(int k=0; k<sysDegree; k++)
						sysNodeInfo2[i].setMessage(sysInitialLLR[i], k, 2);
				}
			}
			...
			for(int i=0; i<sysNumber; i++)
				sysNodeInfo2[i].computeMessage(codedNodeInfo2, sysInitialLLR[i],0, 2);	
			for(int i=0; i<codedNumber; i++) 
				codedNodeInfo2[i].computeMessage(sysNodeInfo2, codedChannelOutput[i], 2);
