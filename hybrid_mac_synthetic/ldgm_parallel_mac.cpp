...
int main() 
{
  // Source info
	int sysNumber = 30000;
	int codedNumber = 60000;
	int codedNumberP = 15000;
  int sysDegree = 10;
	int sysDegreeP = 60;
  ...
  double codeRate = double(sysNumber) / (sysNumber + codedNumber + codedNumberP);
  ...
  cout<<"channel capacity is: "<<capacity<<endl;
	...
	vector<multimap<int, int> > maData2 = paritycm(sysNumber, codedNumberP, sysDegreeP, sysNumber, 50);
  ...
  vector<Sys_info> sysNodeInfo2(sysNumber);
	...
	// Parallel Coded Nodes.
	vector<Coded_info> codedNodeInfo1P(codedNumberP);
	vector<Coded_info> codedNodeInfo2P(codedNumberP);
	...
	for(int index=0;index<sysNumber;++index) {
		...
		sysNodeInfo1[index].readNeighbourNum(onePositionByRow2, index, 2);
	...
	for(int index=0; index<codedNumberP; index++) {
		codedNodeInfo1P[index].readNeighbourNum(onePositionByColumn2, index);
		codedNodeInfo2P[index].readNeighbourNum(onePositionByColumn2, index);
	}

	// Each Sys node get the storage index of its connections.
  for(int index=0;index<sysNumber;++index) {
		sysNodeInfo1[index].getsourceNum(codedNodeInfo1, codedNodeInfo1P, index);
		sysNodeInfo2[index].getsourceNum(codedNodeInfo2, codedNodeInfo2P, index);
	}
	...
	for(int index=0; index<codedNumber; index++) {
 	...
 	for(int index=0; index<codedNumberP; index++) {
		codedNodeInfo1P[index].getsourceNum(sysNodeInfo1, index, 2);
		codedNodeInfo2P[index].getsourceNum(sysNodeInfo2, index, 2);
	}

	/*
	set<int> codedDegree, codedDegreeP;
	for(int i=0; i<codedNumber; i++) {
		codedDegree.emplace_back(codedNodeInfo1[i].nodeData.degree);
		codedDegree.emplace_back(codedNodeInfo2[i].nodeData.degree);
	}
	for(int i=0; i<codedNumberP; i++) {
		codedDegreeP.emplace_back(codedNodeInfo1P[i].nodeData.degree);
		codedDegreeP.emplace_back(codedNodeInfo2P[i].nodeData.degree);
	}

	cout << codedDegree.size() << " " << codedDegreeP.size() << endl;
	for(int i=0; i<codedDegree.size(); i++)
		cout << codedDegree[i] << " ";
	cout << endl;
	for(int i=0; i<codedDegreeP.size(); i++)
		cout << codedDegreeP[i] << " ";
	cout << endl;*/
  ...
  vector<int> systematicBits2(sysNumber, 0);
	...
	vector<int> codedBits2(codedNumber, 0);
 	vector<int> codedBits1P(codedNumberP, 0);
	vector<int> codedBits2P(codedNumberP, 0);
	...
	vector<double> codedChannelOutputP(codedNumberP, 0);
  ...
  int currentBlock = 0;
    ...
    // LDGM encoder
		...
		encoder(codedNodeInfo1P, systematicBits1, codedBits1P);
		encoder(codedNodeInfo2P, systematicBits2, codedBits2P);
    ...
    for(int i=0; i<sysNumber; i++) {
      ...
      double digitSignalA = (2 * codedBits1[i] - 1) * Esender;
		...
		for(int i=0; i<codedNumberP; i++) {
			double digitSignalA = (2 * codedBits1P[i] - 1) * Esender;
			double digitSignalB = (2 * codedBits2P[i] - 1) * Esender;
			channelSignals.push_back(digitSignalA + digitSignalB);
		}
    ...
    for(int index=0;index<sysNumber;++index) {
      ...
      double v0 = log((exp(-pow(y + 2 * sqrt(Esender), 2) / (2 * pow(sigma, 2))) \
			...
			/*
			double pSum0 = 0.5 * (1 - p) * gaussianFunc(y, 0, sigma);            // p0
      double pSum1 = p * gaussianFunc(y, 1/sqrt(2), sigma);    						 // p1
      double pSum2 = 0.5 * (1 - p) * gaussianFunc(y, sqrt(2), sigma);      // p2

			sysChannelOutput[index] = log((pSum0 + 0.5 * pSum1) / (pSum2 + 0.5 * pSum1));
			*/
      ...
      sysNodeInfo2[index].clearMessage();
   	...
   	// Initialization for coded nodes.
		for(int index=0; index<codedNumber; ++index) {
			...
			double pSum1 = gaussianFunc(y, 1 / sqrt(2), sigma);
			double pSum2 = gaussianFunc(y, sqrt(2), sigma);

			codedChannelOutput[index] = log((pSum0 + 0.5 * pSum1) / (pSum2 + 0.5 * pSum1));
			*/
		...
		for(int index=0; index<codedNumberP; ++index) {
			double y = channelSignals[sysNumber + codedNumber + index];
			double v0 = log((exp(-pow(y + 2 * sqrt(Esender), 2) / (2 * pow(sigma, 2))) \
								+	exp(-pow(y, 2) / (2 * pow(sigma, 2)))) /	\
								(exp(-pow(y - 2 * sqrt(Esender), 2) / (2 * pow(sigma, 2))) \
								+ exp(-pow(y, 2) / (2 * pow(sigma, 2)))));
			codedChannelOutputP[index] = v0;
			...
			*/
			...
			codedNodeInfo1P[index].clearMessage();
			codedNodeInfo2P[index].clearMessage();
		}
    ...
    while(currentTime != iterationtime+1) {
      ...
      ++currentTime;
			...
			for(int i=0; i<codedNumberP; i++)
				codedNodeInfo1P[i].computeMessage(sysNodeInfo1, codedChannelOutputP[i], 2);

			for(int i=0; i<sysNumber; i++)
				sysNodeInfo1[i].computeMessage(codedNodeInfo1, codedNodeInfo1P, sysChannelOutput[i] , sideInfo[i]);
			...
			for(int i=0; i<codedNumberP; i++) {
				double y = channelSignals[sysNumber + codedNumber + i];
				double estimate = codedNodeInfo1P[i].message_to_state;
				double nominator1 = exp(-pow(y,2) / (2*pow(sigma, 2))) / exp(estimate);
				double nominator2 = (exp(-pow(y+2*sqrt(Esender), 2) / (2 * pow(sigma, 2))));
				double denominator1 = exp(-pow(y-2*sqrt(Esender), 2) / (2 * pow(sigma, 2))) / exp(estimate);
				double denominator2 = (exp(-pow(y, 2) / (2 * pow(sigma, 2))));
				...
				if(isinf(nominator1) && isinf(denominator1)) {
					codedChannelOutputP[i] = log(exp(-pow(y, 2) / (2 * pow(sigma, 2))) / exp(-pow(y - 2 * sqrt(Esender), 2) / (2 * pow(sigma, 2))));
				} else {
					codedChannelOutputP[i] = log((nominator1 + nominator2) / (denominator1 + denominator2));
				}
			...
			for(int i=0; i<codedNumberP; i++)
				codedNodeInfo2P[i].computeMessage(sysNodeInfo2, codedChannelOutputP[i], 2);

      for(int i=0; i<sysNumber; i++)
				sysNodeInfo2[i].computeMessage(codedNodeInfo2, codedNodeInfo2P, sysChannelOutput[i] , sideInfo[i]);
			...
			for(int i=0; i<codedNumberP; i++) {
				double y = channelSignals[sysNumber + codedNumber + i];
				double estimate = codedNodeInfo2P[i].message_to_state;
				double nominator1 = exp(-pow(y,2) / (2*pow(sigma, 2))) / exp(estimate);
				double nominator2 = (exp(-pow(y+2*sqrt(Esender), 2) / (2 * pow(sigma, 2))));
				double denominator1 = exp(-pow(y-2*sqrt(Esender), 2) / (2 * pow(sigma, 2))) / exp(estimate);
				double denominator2 = (exp(-pow(y, 2) / (2 * pow(sigma, 2))));
