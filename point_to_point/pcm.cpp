...
vector<multimap<int, int> > paritycm(int sysNo,int codedNo,int sysDegree, int numWithSysDegree, int seed)
{
  ...
  vector<int> columnNumberRec(columnNumber,0);//record of one number in each column
  ...
  for(int row=0;row!=rowNumber;++row)
    {
      ...
      // to see if it satisfies the necessary number.
      ...
      if(colRemain < rowConstraint[row])
	{
	  ...
	  // Remove one full column from the full list because of the adjustment of one.
	  ...
	  for(multimap<int,int>::iterator itPro = equal1.first;itPro != equal2.second; ++itPro)
	    {
	      int successFlag=1;
	  ...
	  itDel = equal1.first;
	  while( itDel->second != colProvider)
	    {
	      ++itDel;
	    }
